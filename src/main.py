...
from .config import CLEANUP_INTERVAL, MAX_AGE_DAYS, MAX_FILES, OUTPUT_DIR
from .controllers.web_controller import router as web_router

# Ensure registry is initialized
from .models import get_field_registry_manager
from .utils import ConcurrencyLimitMiddleware, RateLimitMiddleware, RequestSizeLimitMiddleware, perform_cleanup
...
# --- Middleware ---
app.add_middleware(RateLimitMiddleware, rate_limit_per_minute=10, rate_limit_window=60, protected_routes=["/generate"])
app.add_middleware(ConcurrencyLimitMiddleware, max_concurrent_requests=5, timeout=5.0, protected_routes=["/generate"])
app.add_middleware(

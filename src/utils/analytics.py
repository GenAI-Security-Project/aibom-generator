    ...
    try:
        ...
        def _push_log():
            try:
                ...
                try:
                    existing_ds = load_dataset(HF_REPO, token=HF_TOKEN, split="train", trust_remote_code=True)  # nosec B615
...
def get_sbom_count() -> str:
    ...
    if not HF_TOKEN:
        return "N/A"
    try:
        ds = load_dataset(HF_REPO, token=HF_TOKEN, split="train", trust_remote_code=True)  # nosec B615
        return f"{len(ds):,}"
    except Exception as e:

    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if any(request.url.path.startswith(route) for route in self.protected_routes):
            ...
            self.ip_requests[client_ip] = [
                t for t in self.ip_requests[client_ip] if current_time - t < self.rate_limit_window
            ]
...
class ConcurrencyLimitMiddleware(BaseHTTPMiddleware):
    def __init__(self, app, max_concurrent_requests=5, timeout=5.0, protected_routes=None):
        super().__init__(app)
        ...
        try:
            ...
            if any(request.url.path.startswith(route) for route in self.protected_routes):
                try:
                    ...
                    return JSONResponse(
                        status_code=503, content={"detail": "Server is at capacity. Please try again later."}
                    )
            else:
            ...
            return JSONResponse(status_code=500, content={"detail": f"Internal server error in middleware: {str(e)}"})

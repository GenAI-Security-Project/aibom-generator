...
class LocalSummarizer:
    ...
    def _load_model(cls):
        ...
        if cls._model is None:
            try:
                ...
                cls._tokenizer = AutoTokenizer.from_pretrained(cls._model_name)  # nosec B615
                cls._model = AutoModelForSeq2SeqLM.from_pretrained(cls._model_name)  # nosec B615
    ...
    def _extract_candidates(text: str) -> list[str]:
        ...
        heading_matches = re.finditer(
            r"^#+\s*(?:\d+[\.\)]?\s*)?"
            r"(Description|Model [dD]escription|Model Overview|Overview|Introduction|Summary|モデル概要|Model Details)"
            r"[^\n]*\n(.*?)(?=\n#+\s|\Z)",
            text,
        ...
        inline_matches = re.finditer(
            r"(?:Description:|Overview:|### Description:)\s*(.*?)(?=\n\n|\Z)", text, flags=re.DOTALL | re.IGNORECASE
        )
        ...
        tuned_matches = re.finditer(
            r"^(?:The .*model is a .*|This model is a fine-tuned version of.*|This is a fine-tuned.*)",
            text,
    ...
    def _score_candidate(text: str) -> float:
        ...
        if (
            "is a" in text_lower
            or "fine-tuned version of" in text_lower
            or "trained on" in text_lower
            or "designed for" in text_lower
        ):
            score += 20.0
        ...
        if (
            ...
            or "how to run" in text_lower
            or "pip install" in text_lower
            or "read our guide" in text_lower
        ):
            score -= 30.0
    ...
    @classmethod
    def _generate(cls, prompt: str, max_output_chars: int) -> str | None:
        if cls._model is None:
            cls._load_model()
    ...
    def _is_valid_summary(summary: str, model_id: str) -> bool:
        ...
        if (
            summary_lower.startswith("to install")
            or summary_lower.startswith("how to")
            or "pip install" in summary_lower
        ):
            return False
    ...
    @classmethod
    def summarize(cls, text: str, max_output_chars: int = 332, model_id: str = "") -> str | None:
        """
        ...
        prompt1 = (
            f"In one sentence, explain what this AI model is designed to do based on this description:\n\n{short_text}"
        )

...
from .schemas import ConfidenceLevel, DataSource, ExtractionResult
...
class EnhancedExtractor:
    ...
    def __init__(
        ...
        model_file_extractors: list[ModelFileExtractor] | None = None,
    ):
        self.hf_api = hf_api or HfApi()
        self.extraction_results = {}
        self.model_file_extractors = (
            model_file_extractors if model_file_extractors is not None else default_extractors()
        )
    ...
    def _detect_license_from_file(self, model_id: str) -> str | None:
        """
        ...
        for filename in license_filenames:
            try:
                file_path = hf_hub_download(repo_id=model_id, filename=filename)  # nosec B615
    ...
    def _registry_driven_extraction(
        self,
        model_id: str,
        model_info: dict[str, Any],
        model_card: ModelCard | None,
        ...
        logger.info(
            f"📊 Registry extraction complete: {successful_extractions} successful, {failed_extractions} failed"
        )
        ...
        for extractor in self.model_file_extractors:
            try:
                if extractor.can_extract(model_id):
                    ...
                    if metadata:
                        logger.info(f"{type(extractor).__name__} returned {len(metadata)} fields")
                        return metadata
            except Exception as e:
                logger.warning(f"Model file extraction failed ({type(extractor).__name__}): {e}")
                continue
        return {}
    ...
    def _extract_registry_field(self, field_name: str, field_config: dict[str, Any], context: dict[str, Any]) -> Any:
        ...
        if field_name == "license":
            logger.warning("DEBUG: Extracting license...")
    ...
    def _extract_paper_link(self, info: Any) -> str | list[str] | None:
    ...
    def _try_api_extraction(self, field_name: str, context: dict[str, Any]) -> Any:
        ...
        api_mappings = {
            ...
            "license": lambda info: (
                getattr(info.card_data, "license", None) if hasattr(info, "card_data") and info.card_data else None
            ),
            "licenses": lambda info: (
                getattr(info.card_data, "license", None) if hasattr(info, "card_data") and info.card_data else None
            ),
            "datasets": lambda info: (
                getattr(info.card_data, "datasets", []) if hasattr(info, "card_data") and info.card_data else []
            ),
    ...
    def _legacy_extraction(
        self, model_id: str, model_info: dict[str, Any], model_card: ModelCard | None
    ) -> dict[str, Any]:
        """
    ...
    def _generate_external_references(self, model_id: str, metadata: dict[str, Any]) -> dict[str, Any]:
        ...
        external_refs.append({"type": "website", "url": repo_url, "comment": "Model repository"})

        # Model files
        files_url = f"https://huggingface.co/{model_id}/tree/main"
        external_refs.append({"type": "distribution", "url": files_url, "comment": "Model files"})

        # Commit URL if available
        if "commit" in metadata:
            commit_url = f"https://huggingface.co/{model_id}/commit/{metadata['commit']}"
            external_refs.append({"type": "vcs", "url": commit_url, "comment": "Specific commit"})
                ...
                for dataset in datasets:
                    if isinstance(dataset, str):
                        ...
                        external_refs.append(
                            {"type": "distribution", "url": dataset_url, "comment": f"Training dataset: {dataset}"}
                        )
    ...
    def _layer3_stp_extraction(self, model_card: ModelCard | None, model_id: str) -> dict[str, Any]:
    ...
    def _download_and_parse_config(self, model_id: str, filename: str) -> dict[str, Any] | None:
    ...
    def _get_readme_content(self, model_card: ModelCard | None, model_id: str) -> str | None:

...
class FieldRegistryManager:
    ...
    def _load_registry(self) -> dict[str, Any]:
        ...
        try:
            ...
            raise FileNotFoundError(f"Field registry not found at: {self.registry_path}") from None
        except json.JSONDecodeError as e:
            raise ValueError(f"Invalid JSON in field registry: {e}") from e
        except Exception as e:
            raise Exception(f"Failed to load field registry: {e}") from e
    ...
    def get_configurable_scoring_weights(self) -> dict[str, Any]:
        ...
        weights = {
            "tier_weights": scoring_config.get("tier_weights", {"critical": 3, "important": 2, "supplementary": 1}),
            "category_weights": scoring_config.get(
                "category_weights",
                {
                    "required_fields": 20,
                    "metadata": 20,
            ...
            "algorithm_config": scoring_config.get(
                "algorithm_config", {"type": "weighted_sum", "max_score": 100, "normalization": "category_based"}
            ),
        }
    ...
    def analyze_aibom_completeness(self, aibom: dict) -> dict[str, Any]:
        ...
        """
        results = {"category_scores": {}, "total_score": 0, "field_details": {}, "summary": {}}
    ...
    def get_field_jsonpath(self, field_name: str) -> str | None:
    ...
    def get_fields_by_category(self, category: str) -> list[str]:
        ...
        fields = self.get_field_definitions()
        return [field_name for field_name, field_config in fields.items() if field_config.get("category") == category]

    def get_fields_by_tier(self, tier: str) -> list[str]:
        """Get all field names in a specific tier"""
        fields = self.get_field_definitions()
        return [field_name for field_name, field_config in fields.items() if field_config.get("tier") == tier]
    ...
    def validate_registry_integrity(self) -> dict[str, Any]:
        ...
        try:
            ...
            for field_name, field_config in fields.items():
                ...
                if missing_props:
                    validation_results["errors"].append(f"Field '{field_name}' missing properties: {missing_props}")

    ...
    if os.path.exists(schemas_dir):
        for filename in os.listdir(schemas_dir):
            if filename.endswith(".json"):
                with open(os.path.join(schemas_dir, filename), encoding="utf-8") as schema_file:
...
def check_field_with_enhanced_results(
    aibom: dict[str, Any], field: str, extraction_results: dict[str, Any] | None = None
) -> bool:
    """
...
def calculate_completeness_score(
    aibom: dict[str, Any], validate: bool = True, extraction_results: dict[str, Any] | None = None
) -> dict[str, Any]:
    """
    ...
    missing_fields: dict[str, list[str]] = {"critical": [], "important": [], "supplementary": []}
    fields_by_category = {category: {"total": 0, "present": 0} for category in max_scores.keys()}
    field_checklist = {}
    field_types = {}
    field_reference_urls = {}
    category_fields_list: dict[str, list[dict[str, Any]]] = {category: [] for category in max_scores.keys()}
    ...
    for field, classification in FIELD_CLASSIFICATION.items():
        ...
        if not is_gguf or is_present:
            ...
            tier_display = {"critical": "Critical", "important": "Important", "supplementary": "Supplementary"}.get(
                tier, "Unknown"
            )

            category_fields_list[category].append({"name": field, "tier": tier_display, "path": display_path})
    ...
    for category, counts in fields_by_category.items():
        ...
        if counts["total"] > 0:
            percentage = float((counts["present"] / counts["total"]) * 100)
...
def _validate_ai_requirements(aibom: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if "bomFormat" in aibom and aibom["bomFormat"] != "CycloneDX":
        issues.append(
            {"severity": "error", "code": "INVALID_BOM_FORMAT", "message": "Must be CycloneDX", "path": "$.bomFormat"}
        )
...
def validate_aibom(aibom: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if os.path.exists(schema_path):
            ...
            issues.append(
                {"severity": "warning", "message": f"Schema file not found: {schema_file}, skipping strict validation."}
            )
